...
def accept_or_resample(draft_id, x, y, eos_token_id=None, return_stats=False):
    """
    x, y: numpy arrays of probs (same length)
    returns (accepted, token_id, rejection_prob(only if return_stats), y_d_lt_x_d(only if return_stats))
    """
    ...
    if eos_token_id is not None and draft_id == eos_token_id:
        ...
        if return_stats:
            return True, draft_id, 0.0, False
        else:
    ...
    if random.random() < y_d / x_d:
        if return_stats:
            return True, draft_id, (1 - y_d / x_d), True
        else:
            return True, draft_id
    else:
        ...
        if np.sum(delta) == 0:
            # fallback uniform
            if return_stats:
                return False, int(np.argmax(y)), (1 - y_d / x_d), True
            else:
                return False, int(np.argmax(y))
        delta /= np.sum(delta)
        token_id = int(np.random.choice(len(delta), p=delta))
        if return_stats:
            return False, token_id, (1 - y_d / x_d), True
        else:
